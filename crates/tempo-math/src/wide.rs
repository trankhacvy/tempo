...
//! operator configs. `mul_div_floor`/`mul_div_ceil` compute `floor|ceil(a·b/d)`
//! exactly via a 256-bit product, with a fast `u128` path for the common case.
...
/// or `None` if the quotient would not fit in `u128` (i.e. `hi >= d`).
...
mod tests {
    ...
    fn test_small_cases() {
        ...
        assert_eq!(mul_div_floor(10, 3, 5), Some(6));
