...
/// Accounts for the ProposeSetOracle instruction (stage-only: no event until apply).
///
/// # Account Layout
/// 0. `[signer]` authority - must match `Market.authority`
/// 1. `[writable]` market
pub struct ProposeSetOracleAccounts<'a> {
    pub authority: &'a AccountView,
    pub market: &'a AccountView,
}

impl<'a> TryFrom<&'a [AccountView]> for ProposeSetOracleAccounts<'a> {
...
impl<'a> InstructionAccounts<'a> for ProposeSetOracleAccounts<'a> {}
