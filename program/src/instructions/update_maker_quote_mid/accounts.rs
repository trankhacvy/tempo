...
/// Accounts for the UpdateMakerQuoteMid instruction (the hot path).
///
/// # Account Layout
/// 0. `[signer]` writer - the maker or its delegate
/// 1. `[]` market - supplies `num_ticks` for the bound check
/// 2. `[writable]` maker_quote
pub struct UpdateMakerQuoteMidAccounts<'a> {
    pub writer: &'a AccountView,
    pub market: &'a AccountView,
    pub maker_quote: &'a AccountView,
}

impl<'a> TryFrom<&'a [AccountView]> for UpdateMakerQuoteMidAccounts<'a> {
...
impl<'a> InstructionAccounts<'a> for UpdateMakerQuoteMidAccounts<'a> {}
