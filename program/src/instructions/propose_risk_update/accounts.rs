...
/// Accounts for the ProposeRiskUpdate instruction (stage-only: no event until apply).
///
/// # Account Layout
/// 0. `[signer]` authority - must match `Market.authority`
/// 1. `[writable]` market
pub struct ProposeRiskUpdateAccounts<'a> {
    pub authority: &'a AccountView,
    pub market: &'a AccountView,
}

impl<'a> TryFrom<&'a [AccountView]> for ProposeRiskUpdateAccounts<'a> {
...
impl<'a> InstructionAccounts<'a> for ProposeRiskUpdateAccounts<'a> {}
