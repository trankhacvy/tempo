//! Plan task 0.3 — after a sequence of submits and cancels, both
//! `Market.active_order_count` and `OrderSlabHeader.count` must equal the number
//! of orders still resting in the slab.
...
fn counts_track_resting_orders_through_submits_and_cancels() {
    ...
    assert_eq!(ctx.market(&pdas).active_order_count, 5);
    ...
    assert_eq!(ctx.market(&pdas).active_order_count, 3);
    ...
    // The on-chain counters must agree with the actual resting set.
    assert_eq!(ctx.market(&pdas).active_order_count, resting.len() as u64);
